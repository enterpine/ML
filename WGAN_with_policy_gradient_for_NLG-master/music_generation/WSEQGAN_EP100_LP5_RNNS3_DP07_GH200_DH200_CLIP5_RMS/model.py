...
class SeqGAN(object):
    ...
    def build_generator(self, use_multinomial=False, num_rnns=3, rnn_dims=200):
    ...
    def get_discriminator_op(self, r_preds, g_preds, d_weights):
        ...
        with tf.variable_scope('loss/discriminator'):
            ...
            clip_D = [p.assign(tf.clip_by_value(p, -5, 5)) for p in d_weights]  ######
            tf.summary.scalar('total', total_loss)

        return dis_op, clip_D ######
    ...
    def get_generator_op(self, g_sequence, d_preds, g_preds, g_weights):
        ...
        with tf.variable_scope('loss/generator'):
            ...
            # Creates the optimizer.
            generator_opt = tf.train.RMSPropOptimizer(1e-4) ######
    ...
    def build(self, reg_loss=1e-4):
        ...
        dis_op, clip_D = self.get_discriminator_op(r_preds, g_preds, d_weights) ######
    ...
    def train_batch(self, batch):
        ...
        if t % self.log_every:
            self._sess.run([self.train_op, self.clip_D], feed_dict=feed_dict) ######
        else:                                                                 
            _, _, summary = self._sess.run([self.train_op, self.clip_D, self.summary_op], 
                                        feed_dict=feed_dict)
